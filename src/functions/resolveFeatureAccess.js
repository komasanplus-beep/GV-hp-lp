...
Deno.serve(async (req) => {
  try {
    ...
    if (!feature_code) {
      return Response.json(
        { error: 'feature_code is required' },
        { status: 400 }
      );
    }
    ...
    const features = await base44.asServiceRole.entities.FeatureMaster.filter({
      code: feature_code
    });
    ...
    if (subscription_status === 'canceled' || subscription_status === 'expired') {
      ...
      // past_due の場合、閲覧のみ可、新規作成は不可という制限が一般的
      // 新規作成系の機能（site_builder, page_management等）はブロック
      ...
      if (paused_blocked_features.includes(feature_code)) {
        subscription_blocks = true;
    ...
    // ===== 5. FeatureGrant による個別 disable / enable =====
    // disable を優先的にチェック
    const disableGrants = await base44.asServiceRole.entities.FeatureGrant.filter({
      feature_code: feature_code,
      grant_type: 'disable',
      status: 'active'
    });
    ...
    for (const grant of disableGrants) {
      // 有効期限チェック
      if (grant.end_at && new Date(grant.end_at) < new Date()) {
        continue;
      }

      // user, site, tenant のいずれかにマッチするか
      if (
    ...
    // enable grant をチェック
    const enableGrants = await base44.asServiceRole.entities.FeatureGrant.filter({
      feature_code: feature_code,
      grant_type: 'enable',
      status: 'active'
    });
    ...
    if (site_id) {
      ...
      if (sites.length > 0) {
        const site = sites[0];
        // site.enabled_features の legacy フィールドをチェック
        const siteEnabledFeatures = site.enabled_features || {};
        
        // booking, blog, customer, inquiry を Site レベルで設定済みならそれを使う
        ...
        if (siteKey && siteKey in siteEnabledFeatures) {
          if (siteEnabledFeatures[siteKey] === true) {
            return Response.json({
              allowed: true,
          ...
          } else if (siteEnabledFeatures[siteKey] === false) {
            return Response.json({
              allowed: false,
              source: 'site_override',
              subscription_status: subscription_status,
    ...
    // ===== 7. PlanMaster.included_features =====
    ...
    // ===== 8. FeatureMaster.default_enabled =====
